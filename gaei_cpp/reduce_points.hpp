...
void remove_trivial_surface(const std::vector<size_t>& lc, std::vector<vertex<>>& vs)
{
...
void remove_minor_labels(std::vector<size_t> lc, std::vector<vertex<>>& vs, size_t threshold = 5) noexcept
{
    vs.erase(std::remove_if(vs.begin(), vs.end(),
